...
public static class SpawnPool
{
        ...
        Transform parent = null, int poolSize = 1)
    {
        ...
        // Kiểm tra các object trong pool có còn hợp lệ không
        while (_pools[prefab].Count > 0)
        {
            var obj = _pools[prefab].Peek();
            if (obj != null)
            {
            ...
            {
                // Xóa object đã destroy khỏi queue
                _pools[prefab].Dequeue();
            }
        }
    ...
    {
        if (obj == null) return;  // Kiểm tra null
        if (!_pools.ContainsKey(prefab)) _pools[prefab] = new Queue<GameObject>();
        obj.SetActive(false);
        _pools[prefab].Enqueue(obj);
    }
