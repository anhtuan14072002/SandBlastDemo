...
namespace Crystal
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        [SerializeField] bool TopOnly = false;   // chỉ co phía trên, giữ nguyên đáy
        ...
        void ApplySafeArea (Rect r)
        {
            ...
            if (!ConformY)
            {
                // Bỏ safe area theo trục Y
                r.y = 0;
                r.height = Screen.height;
            }
            else if (TopOnly)
            {
                // CHỈ dùng safe area phía trên, bỏ phần home bar dưới

                // r.y   = bottom inset (khoảng hở dưới, ví dụ 102 trên iPhone X)
                // r.y+r.height = Screen.height - topInset
                float topInset = Screen.height - (r.y + r.height);

                // kéo rect xuống sát đáy, giữ lại khoảng hở phía trên
                r.y = 0f;
                r.height = Screen.height - topInset;
                // -> bottom = 0, top = Screen.height - topInset (vẫn chừa notch)
            }
