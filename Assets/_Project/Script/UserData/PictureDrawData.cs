...
namespace Sand
{
    public class PictureDrawData
    {
        ...
        {
            if (pictureIndex < 0 || totalRegions <= 0) return 0;
            if (_userData.CompletedPictureIndices.Contains(pictureIndex)) return totalRegions;

            if (_userData.PictureFillProgress != null && _userData.PictureFillProgress.TryGetValue(pictureIndex, out var count))
            {
        ...
        {
            if (pictureIndex < 0 || totalRegions <= 0) return;
